...
export async function planPayout(
  ...
  const rewardAsset = metadata?.rewardAsset ?? "REWARD";
...
export async function settlePayout(epochId: string, wallet: string, txSig: string, rewardAsset = "REWARD") {
  const result = await supabase
    .from("payouts")
...
export async function failPayout(epochId: string, wallet: string, error: unknown, rewardAsset = "REWARD") {
  const result = await supabase
    .from("payouts")
    .update({
      status: "failed",
