...
export async function runEpoch(date = new Date()) {
  ...
  try {
    ...
    console.log(`[${epochId}] selected ANSEM reward recipients from BEG holders: ${holders.length}`);
    ...
    const payoutReserveLamports = await estimateTokenPayoutReserveLamports([
      { wallets: holders.map((holder) => holder.wallet), mint: config.rewardTokenMint, label: "ANSEM-to-BEG-holders" },
    ...
    console.log(
      `[${epochId}] BEG pool: ${indexPoolRaw.toString()} raw of ${availableIndexRaw.toString()} raw treasury balance (${config.airdropRewardBps} bps)`
    );
    ...
    if (!allocations.length && !indexAllocations.length) {
      ...
      console.log(`[${epochId}] no ANSEM or BEG reward balance, skipped airdrop`);
      return;
    }
    ...
    const indexAirdrop = indexAllocations.length
      ? await airdropTokenRewards(epochId, indexAllocations, "BEG", config.sourceTokenMint)
      : { settledUi: 0, settledCount: 0, stoppedForReserve: false };
    if (indexAirdrop.stoppedForReserve && indexAirdrop.settledCount === 0) {
      throw new Error("BEG airdrop stopped before sending any payouts: treasury SOL below airdrop reserve");
    }
