...
console.log(`HyperHood worker started. Schedule: every ${config.epochMinutes} minutes.`);
console.log(
...
console.log(
  `Reward split: ${config.swapBalanceBps / 100}% claimed SOL buys HoodX for eligible holder airdrops; ${config.sideWalletBps / 100}% routes to side wallet.`
);
console.log(`Source token mint: ${config.sourceTokenMint.toBase58()}`);
console.log(`Eligibility gate: holder must hold ${config.eligibilityMin.toLocaleString()} source tokens`);
