...
function replyText(tweet: XTweet) {
  ...
  if (lower.includes("treasury")) return "Inuvestors sees the treasury question. Every serious desk watches cash before chasing candles.";
  if (lower.includes("buy")) return "Inuvestors only moves from approved signals. No random paws on the wallet.";
  if (lower.includes("mission") || lower.includes("bounty")) return "Market desk missions are waking up. Bring useful work, not noise.";
  return "Inuvestors saw this. The desk is learning the room.";
}
